# FinOps Guide: Token Cost Optimization for Context Engineering

## Executive Summary

This guide provides actionable strategies to reduce token burn in the context-engineering skill while maintaining output quality. By implementing the patterns below, teams can achieve **30-50% token reduction** with minimal quality impact.

### Quick Wins (Implement Today)
| Action | Tokens Saved/Session | Cost Savings/Session | Effort |
|--------|---------------------|---------------------|--------|
| Compress failed attempts to one-liners | 300-800 | $0.0015-$0.004 | Low |
| Replace verbose tool output with single-line summaries | 200-500 | $0.001-$0.0025 | Low |
| Eliminate conversational filler | 500-1500 | $0.0025-$0.0075 | Medium |
| Use MCP on-demand instead of upfront loading | ~2,400 | ~$0.012 | Low |

**Total potential savings: $0.03 - $0.08 per session**
...
## 2. Aggressive Compression Patterns
...
1. Start fresh session with handoff summary (see Session Boundary Token Savings)
...
- [ ] Implement Pattern A (tool output compression)
- [ ] Implement Pattern B (failed attempts compression)
- [ ] Switch MCP to on-demand queries
- [ ] Set up token budget monitoring at 70% threshold

### Phase 2: Medium-Term Optimizations (Weeks 2-4)
- [ ] Implement Pattern C (stack trace compression)
- [ ] Implement Pattern D (file listing compression)
- [ ] Implement Pattern E (conversational filler removal)
- [ ] Set up cost tracking dashboards

### Phase 3: Advanced Optimization (Month 2+)
- [ ] Implement tiered compression strategy
- [ ] Automate emergency compression protocols
- [ ] Build ROI measurement tooling
- [ ] Establish monthly FinOps review cadence

---

## Appendix: Cost Reference Table
